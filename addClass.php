    ...
    <script>
       ...
       function changeStyle(id){
         ...
         if(cb_input.checked){
           ...
           cb_img.src = 'images/Icons/menos.svg'
         }else{
           document.getElementById('cb_lb'+id).style.color = '#BAE6E9';
           cb_img.src = 'images/Icons/mais.svg'
         }
  ...
  <body>
    <section class="window">
      ...
      <div class="container all container_extended">
        <div class="title">ADICIONAR TURMA POR CSV</div>
        <div class="inner_container contour">
          ...
          <a href="students_class.csv" download="Modelo Turma CSV(não modificar a formatação).csv"><input class="button button_file" type="button" value="Baixar Modelo CSV" /></a>
          <a class="button" href="#divList" onclick="cadastraCSV()">CADASTRAR CSV</a>
        </div>
          <form action="setAddClass.php" method="post">
            <div class="title">CADASTRO DE TURMA</div>
              <div class="inner_container">
                <input type="text" class="input_text" placeholder="Digite o nome da turma" name="class_name" required>

                <input type="text" class="input_text" placeholder="Digite o curso da turma" name="course_class" required>
                ...
                <?php
                  ...
                  if(isset($_GET['id_students'])){
                    for($i = 0; $i < count($_GET['id_students']); $i++){
                      if(!in_array($_GET['id_students'][$i], $students)){
                        array_push($students, $_GET['id_students'][$i]);
                      }
                    }
                  }
                  ...
                  for ($i = 0; $i < count($rows); $i++) {
                    ...
                    echo '>';
                    echo '<label for = "cb'.$rows[$i]['id_aluno'].'"><img id="icon'.$id_aluno.'" class="cb_icon" src="images/Icons/mais.svg"></ label>';
                    echo '</td>';
